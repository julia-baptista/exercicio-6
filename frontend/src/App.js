...
function App() {
  ...
  socket.on("symbolO", (symbol) => {
    setSymbol("O");
  });

  socket.on("symbolX", (symbol) => {
    setSymbol("X");
  });
  ...
  // function clickP0() {
  //   if (!lock && p0 === "") {
  //     socket.emit("p0", { symbol, p0, privateRoom });
  //     setP0(symbol);
  //     setLock(true);
  //   }
  // }
  ...
  function X() {
    ...
    setPrivateRoom(parseInt(Math.random()*1000));
  }
  ...
  return (
    <div className="App">
      ...
      {symbol && (
        <div>
          <div>O seu símbolo é o {symbol}</div>
          <p></p>
          <table>
            <tr>
      ...
      {!symbol && (
        <div>
          ...
          <span onClick={X}>X</span> ou <span onClick={O}>O</span>
        </div>
      )}
    </div>
  );
